...
class ChildViewController3: UIViewController, IndicatorInfoProvider,UICollectionViewDataSource,UICollectionViewDelegate {
    var events:[String]=[]
    var eventName:[String] = []
    var iconName:[String] = []
    var timings:[String]=[]
    @IBOutlet weak var collectionDay3: UICollectionView!
    ...
    func fetchFirebaseData(){
        (UIApplication.shared.delegate as! AppDelegate).ref1.child("timelineIOS").child("Day3").observe(.value) { (snapshot) in
            ...
            {
                (UIApplication.shared.delegate as! AppDelegate).ref1.child("timelineIOS").child("Day3").child(name).observe(.value, with: { (snapshot1) in
                    ...
                    self.collectionDay3.reloadData()
