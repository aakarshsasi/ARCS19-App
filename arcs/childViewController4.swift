...
class ChildViewController4: UIViewController, IndicatorInfoProvider,UICollectionViewDataSource,UICollectionViewDelegate {
    var events:[String]=[]
    ...
    func fetchFirebaseData(){
        (UIApplication.shared.delegate as! AppDelegate).ref1.child("timelineIOS").child("Day4").observe(.value) { (snapshot) in
            ...
            {
                (UIApplication.shared.delegate as! AppDelegate).ref1.child("timelineIOS").child("Day4").child(name).observe(.value, with: { (snapshot1) in
    ...
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo(title:"Day 4")
    }
